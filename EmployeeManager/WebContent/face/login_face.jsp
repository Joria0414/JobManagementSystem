		...
		<script type="text/javascript">
  		...
  		//导航 获取用户媒体对象
  			...
  			function query(){
  				...
  				$.ajax({
  					type:"post",
  					url:"${pageContext.request.contextPath}/FaceLoginSvl",
  					data:{"base":base},
  					dataType: "json",
  					success:function(json){
  						 if(json.message==""){ 
  							...
  							window.parent.location.replace("${pageContext.request.contextPath}/loginForm.jsp");
  							 ...
  							 //alert(json.role);
  							if(json.role=='2'){
  								window.parent.location.replace("${pageContext.request.contextPath}/admin_main.jsp");
  							 }else{
  								window.parent.location.replace("${pageContext.request.contextPath}/emp_main.jsp");
  							 }
  						} 
					}
				});
